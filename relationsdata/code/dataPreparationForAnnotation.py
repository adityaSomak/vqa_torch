...
'''
def getRelationsUsingHeuristics(pairInPhrase, phrase, question, relations):
	...
	for idx,rel in enumerate(relations):
		...
		similarities.append((idx,w2vSim))
		if w2vSim > maxSim:
			maxSim = w2vSim;
			bestRelation = rel
	similarities = sorted(similarities, key=lambda i: i[1], reverse=True)
	top10relations = [relations[i[0]] for i in similarities[0:10]]
	return top10relations
		...
		for line in questionsF:
			...
			for pairInPhrase in nounPairsInPhrase:
				...
				if sim > EXISTENCE_THRESHOLD:
					...
					top10relations = getRelationsUsingHeuristics(pairInPhrase, phrase[0], question, allPredicates)
					cleanedData.write(phrase[0]+"\t"+str(getWords(pairInPhrase))+"\t"+ str(top10relations)+"\t"+ question+"\n");
					i=i+1
					if i%1000==0:
						cleanedData.flush()
					if i%5000==0:
...
mt = sys.argv[1];
print mt
questionsFileName = "../sortedqaDependencies.txt"
...
cleanedDataFilePrefix = "../trainingData/relationsForAnnotation";
